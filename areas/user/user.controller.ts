import { readJson, Controller, Get, Post, Put, Delete, Req, Body, Content, InternalServerError, NotFoundError, BadRequestError, UnauthorizedError, ForbiddenError, ServerRequest, genSalt, hash, compare, makeJwt, setExpiration, validateJwt } from "../../deps.ts";
...
export class UserController {
  ...
  private async initUser(fileName: string) {
    const users = await readJson(`${env.currentWorkingDir}/datasets/${fileName}`);
  ...
  private async verifyAuth(headers: Headers, isLogout = false): Promise<StringOrNull> {
    ...
    const tokens = allTokens.map(({ header, payload, signature }) => `${header}.${payload}.${signature}`);
  ...
  async loginUser(@Body() body: { login: string; password: string; }) {
    try {
      ...
      const { _id: { "$oid": id }, ...document } = await this.userService.findUserByLogin(login);
  ...
  async getUser(@Req() req: ServerRequest) {
    try {
      ...
      const { password, _id: { "$oid": id }, ...document } = await this.userService.findUserById(iss); 
